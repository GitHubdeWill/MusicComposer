...
//			out(hashMs.get(track).toString());
	...
	public static void transformFile (String fileName, String targetFileName) {
		String	strSource = fileName;
		String	strFilename = null;
		MidiFileFormat	fileFormat = null;
		Sequence	sequence = null;
...
//			out(hashMs.get(track).toString());
		...
		MidiFileBuilder mBuilder = new MidiFileBuilder(fileFormat.getResolution(), targetFileName);
		for (Long key : keys) { 
		   Chord value = map.get(key);
		   for (int i : value.getNotes()) {
			   mBuilder.addNote(new Note(i, 0, 100, key, fileFormat.getResolution()), 0);
		   }
		   out(key+">>"+value.toString());
		}
